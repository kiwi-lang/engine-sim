#include "../include/ui_cylinder_temperature_gauge.h"
...
CylinderTemperatureGauge::~CylinderTemperatureGauge() { /* void */ }

void CylinderTemperatureGauge::initialize(EngineSimApplication *app) { UiElement::initialize(app); }

void CylinderTemperatureGauge::destroy() { UiElement::destroy(); }
...
void CylinderTemperatureGauge::render()
{
    ...
    for (int i = 0; i < m_engine->getCylinderCount(); ++i)
    {
        ...
        const Bounds &b_cyl = bankGrid.get(b, 0, bank->getCylinderCount() - piston->getCylinderIndex() - 1).inset(5.0f);
